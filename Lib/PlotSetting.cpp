...
//set the general style optioins
void SetSgStyle(){
	//No Canvas Border
...
void txtN(Double_t x0, Double_t y0, TH1 *h, std::string sName, Double_t sizeTxt){
	h->SetStats(0);
	...
	ltx->DrawLatex(x0,y0,Form(sName.c_str(),h->GetEntries()));
	gPad->Modified();
	gPad->Update();
}

TH1F * newTH1F(std::string name, Double_t binw, Double_t LowBin, Double_t HighBin, Bool_t MevTitle, Int_t iMode){
	Int_t nbin=TMath::Nint((HighBin-LowBin)/binw);
	HighBin=binw*nbin+LowBin;
	TH1F *h=new TH1F(name.c_str(),"",nbin,LowBin,HighBin);
