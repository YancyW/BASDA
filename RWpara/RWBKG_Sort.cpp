...
void CBKG_Sort:: Read_BKG_Sort_Test(CPath &path){
	...
	if(bkg_node["sub_sort_name"]){
		input_class_name = bkg_node["sub_sort_name"].as<std::vector<std::string> >();
	}
	if(bkg_node["sub_sort_num"]){
		input_class_vec = bkg_node["sub_sort_num"].as<std::vector<int> >();
	}

	sort.resize(_sort_num);
	for(int i=0;i<_sort_num;i++){
		if(bkg_node[input_class_name[i]]){
			_string_tmp = bkg_node[input_class_name[i]].as<std::vector<std::string> >();
		}
