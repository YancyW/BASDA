...
void Print_Complete_Channel(CDraw &para, float lumi, std::ostream& out, AFile_List &fileList, ASort_List& data){
	...
	Print_Table_Begin(out,"tiny", fileList.File_Num()+5);
	...
	Print_Table_First_Column (out, " S/B", false);
	...
	for(int j=0;j<fileList.Data_Num();j++){
		...
		for(int i=0;i<fileList.File_Num();i++){
			...
			Print_Table_First_Column(out, Calc_Round_Decimal(data_ij,1));

			if(i==fileList.Last_File_Num()){
				float total     = fileList.Data_Bkg(j);
				float signal    = fileList.Data_Sig(j);
				float signal_ori= fileList.Data_Sig(2);
				Print_Table_First_Column(out, Calc_Round_Decimal(total,1));
				Print_Table_First_Column(out, Calc_Round_Decimal(signal/signal_ori,2));
				Print_Table_First_Column(out, Get_S_over_B(signal,total));
...
void Print_Complete_Sort(CDraw &para, std::ostream& out, ASort_List& data){
	...
	Print_Table_Begin(out,"tiny", data.Total_Unzero_Sort_Num()+5);


	Print_Table_First_Column(out, para.scenario.Lumi_Tex(),true,40);
	for(int k=0;k<data.Sort_Num();k++){
		for(int l=0;l<data.Sub_Sort_Num(k);l++){
			data.Set_Data_Pos("no cut MC event",k,l);
			if(data.Data_Content("no cut MC event",k,l)!=0){
				Print_Table_First_Column(out, data.Full_Sort_Name(k,l), true);
			}
		}
	}
		...
		for(int k=0;k<data.Sort_Num();k++){
			for(int l=0;l<data.Sub_Sort_Num(k);l++){
				...
				if(data.Data_Content(pos+1,k,l)!=0){
					Print_Table_First_Column(out, Calc_Round_Decimal(data.Data_Content(pos+j-1,k,l),1 ));
				}
			}
		}
		if( data.Total_Unzero_Sort_Num()>1){
			Print_Table_First_Column(out, Calc_Round_Decimal(data.Data_Bkg(j),1));
			Print_Table_First_Column(out, Calc_Round_Decimal(data.Data_Sig(j)/data.Data_Sig(2),2)); 
			Print_Table_First_Column(out, Get_S_over_B(data.Data_Sig(j),data.Data_Bkg(j)));
