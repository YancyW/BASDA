...
private:                                                                   
    ...
    static auto invoke(TT &&obj, AArgs&& ...args)                              
        -> decltype(std::forward<TT>(obj).Member(std::forward<AArgs>(args)...),std::true_type());                         
...
public:                                                                    
    static const bool value = _has_member && (std::is_void<R>::value || std::is_convertible<_return_type, R>::value);    
    using type = typename                                                      
